...
double FGTable::GetValue(double rowKey, double colKey) const
{
    ...
    while (r > 2     && Data[r-1][0] > rowKey)
    {
        r--;
    }
    while (r < nRows && Data[r]  [0] < rowKey)
    {
        r++;
    }

    while (c > 2     && Data[0][c-1] > colKey)
    {
        c--;
    }
    while (c < nCols && Data[0][c]   < colKey)
    {
        c++;
    }
...
double FGTable::GetValue(double rowKey, double colKey, double tableKey) const
{
    ...
    while (r > 2     && Data[r-1][1] > tableKey)
    {
        r--;
    }
    while (r < nRows && Data[r]  [1] < tableKey)
    {
        r++;
    }
