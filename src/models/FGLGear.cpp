        ...
        Castered(false)
{
    ...
    {
        cerr << "No location given for contact " << name << endl;
        exit(-1);
    }
    ...
    if (element && (eContactType == ctBOGEY))
    {
        ...
        cp=cos(vGearOrient(ePitch));
        sp=sin(vGearOrient(ePitch));
        cr=cos(vGearOrient(eRoll));
        sr=sin(vGearOrient(eRoll));
        cy=cos(vGearOrient(eYaw));
        sy=sin(vGearOrient(eYaw));
